use veyra_party::{AgeEvidence, BookingParty, CivilDate, ConstraintStrength, RoomingIntent, RoomingRelation, Traveler, TravelerId};
...
fn policy(capacity: u16) -> veyra_rule_compiler::CompiledRule {
    compile(RULE_SCHEMA_V1, &Rule::Capacity { min: 1, max: capacity }).unwrap_or_else(|_| unreachable!())
}
...
fn party_with_intent(strength: ConstraintStrength, relation: RoomingRelation) -> BookingParty {
    ...
    builder.add_rooming_intent(RoomingIntent {
        left: TravelerId::new(1), right: TravelerId::new(2), strength, relation,
    }).unwrap_or_else(|_| unreachable!());
    builder.build().unwrap_or_else(|_| unreachable!())
}
...
fn input_bounds_fail_closed() {
    let p = party(1);
    assert_eq!(solve(&p, date(), &[], SolverConfig::default()), Err(SolverError::InvalidRoomCount(0)));
    ...
    assert_eq!(solve(&p, date(), &[negative], SolverConfig::default()), Err(SolverError::NegativeRoomPrice(1)));
    assert_eq!(solve(&p, date(), &[room(1, 1), room(1, 2)], SolverConfig::default()), Err(SolverError::DuplicateRoomId(1)));
    assert!(matches!(solve(&party(17), date(), &[room(1, 1)], SolverConfig::default()), Err(SolverError::TooManyTravelers(17))));
}
...
fn all_solver_errors_have_stable_display_surface() {
    for error in [
        SolverError::EmptyParty, SolverError::TooManyTravelers(17), SolverError::InvalidRoomCount(0),
        SolverError::InvalidBudget, SolverError::InvalidAdultAge(1), SolverError::NegativeRoomPrice(1),
        SolverError::DuplicateRoomId(1), SolverError::StateBudgetExhausted, SolverError::TooManyValidSolutions(2),
        ...
        SolverError::UnknownTraveler(TravelerId::new(99)), SolverError::Price(PricingError::Overflow),
        SolverError::PenaltyOverflow, SolverError::InternalInvariant,
