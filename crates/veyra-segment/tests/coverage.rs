...
fn path(label: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "veyra-segment-coverage-{label}-{}-{nanos}.bin",
        std::process::id()
    ))
}
...
fn header_and_payload_corruption_fail_closed() {
    ...
    let mut bytes = original.clone();
    bytes[24..32].copy_from_slice(&30_u64.to_le_bytes());
    ...
    let mut bytes = original.clone();
    bytes[48..56].copy_from_slice(&79_u64.to_le_bytes());
    assert!(matches!(
        Segment::decode(&bytes),
        Err(SegmentError::InvalidPayloadOffset(79))
    ));
    let mut bytes = original.clone();
    bytes[56..64].copy_from_slice(&(512_u64 * 1024 * 1024 + 1).to_le_bytes());
    assert!(matches!(
        Segment::decode(&bytes),
        Err(SegmentError::PayloadTooLarge(_))
    ));
    let mut bytes = original.clone();
    bytes.pop();
    assert!(matches!(
...
fn file_io_atomicity_and_oversize_guard_are_exercised() {
    ...
    original
        ...
        .unwrap_or_else(|_| unreachable!());
    assert_eq!(
        Segment::read(&target).unwrap_or_else(|_| unreachable!()),
        original
    );

    let missing = path("missing");
    let error = Segment::read(&missing)
        .err()
    ...
    let file = OpenOptions::new()
        ...
        .open(&oversized)
        .unwrap_or_else(|_| unreachable!());
    file.set_len(512_u64 * 1024 * 1024 + 81)
        .unwrap_or_else(|_| unreachable!());
    drop(file);
    assert!(matches!(
        Segment::read(&oversized),
        Err(SegmentError::PayloadTooLarge(_))
    ));

    assert!(matches!(
        original.write_atomic(Path::new("/")),
        Err(SegmentError::MissingFileName)
    ));
    let impossible_parent = path("no-parent").join("missing").join("segment.bin");
    assert!(matches!(
        original.write_atomic(&impossible_parent),
...
fn error_display_source_and_build_validation_are_covered() {
    ...
    for error in [
        ...
        SegmentError::UnknownSegmentType(9),
        SegmentError::TruncatedHeader(3),
        SegmentError::MalformedHeader,
        SegmentError::LsnRangeReversed,
        SegmentError::PayloadTooLarge(4),
    ...
    assert!(io_error.to_string().contains("segment"));
}
