...
impl TupleData {
    /// Canonical internal encoding used by CDC journal/projection handoff. It preserves tuple
    /// column kind and bytes without interpreting PostgreSQL types.
    #[must_use]
    ...
    pub fn decode(input: &[u8]) -> Result<PgOutputMessage, PgOutputError> {
        ...
        let message = match tag {
            ...
            b'I' => {
                ...
                cursor.tag(b'N')?;
                let new_tuple = decode_tuple(&mut cursor)?.encode();
                PgOutputMessage::Change(RowChange::new(
                    relation_id,
                    ChangeKind::Insert,
                    None,
                    Some(new_tuple),
            ...
            b'T' => return decode_truncate(&mut cursor),
...
fn decode_truncate(cursor: &mut Cursor<'_>) -> Result<PgOutputMessage, PgOutputError> {
    let relation_count = cursor.u32_be()? as usize;
    if relation_count == 0 || relation_count > MAX_COLUMNS {
        return Err(PgOutputError::InvalidTruncateCount(relation_count));
    }
    let _options = cursor.u8()?;
    let first_relation = cursor.u32_be()?;
    for _ in 1..relation_count {
        let _ = cursor.u32_be()?;
    }
    cursor.finish()?;
    Ok(PgOutputMessage::Change(RowChange::new(
        first_relation,
        ChangeKind::Truncate,
        None,
        None,
...
impl<'a> Cursor<'a> {
    ...
    fn u16_be(&mut self) -> Result<u16, PgOutputError> {
        let bytes: [u8; 2] = self.take(2)?.try_into().map_err(|_| PgOutputError::UnexpectedEof)?;
        Ok(u16::from_be_bytes(bytes))
    }

    fn u32_be(&mut self) -> Result<u32, PgOutputError> {
        let bytes: [u8; 4] = self.take(4)?.try_into().map_err(|_| PgOutputError::UnexpectedEof)?;
    ...
    fn cstring(&mut self, max: usize) -> Result<String, PgOutputError> {
        ...
        let _nul = self.u8()?;
        let text = core::str::from_utf8(bytes).map_err(|_| PgOutputError::InvalidUtf8)?;
        Ok(text.to_owned())
    }
...
mod tests {
    ...
    fn decodes_begin_and_commit() {
        ...
        assert_eq!(
            ...
            Ok(PgOutputMessage::Commit {
                ...
                commit_timestamp_micros: 50,
            })
        );
    }

    #[test]
    fn decodes_relation_metadata() {
        let mut input = vec![b'R'];
        input.extend_from_slice(&7_u32.to_be_bytes());
        input.extend_from_slice(b"public\0hotels\0");
        input.push(b'd');
        input.extend_from_slice(&1_u16.to_be_bytes());
        input.push(1);
        input.extend_from_slice(b"id\0");
        input.extend_from_slice(&2950_u32.to_be_bytes());
        input.extend_from_slice(&(-1_i32).to_be_bytes());
        let decoded = PgOutputDecoder::decode(&input).unwrap_or_else(|_| unreachable!());
        let PgOutputMessage::Relation(relation) = decoded else { unreachable!() };
        assert_eq!(relation.relation_id, 7);
        assert_eq!(relation.namespace, "public");
        assert_eq!(relation.name, "hotels");
    ...
    #[test]
    fn decodes_insert_update_delete_and_tuple_kinds() {
        let tuple = tuple_text(b"abc");
        let mut insert = vec![b'I'];
        insert.extend_from_slice(&7_u32.to_be_bytes());
        insert.push(b'N');
        insert.extend_from_slice(&tuple);
        let PgOutputMessage::Change(insert) = PgOutputDecoder::decode(&insert).unwrap_or_else(|_| unreachable!()) else { unreachable!() };
        ...
        update.push(b'N');
        update.extend_from_slice(&tuple);
        let PgOutputMessage::Change(update) = PgOutputDecoder::decode(&update).unwrap_or_else(|_| unreachable!()) else { unreachable!() };
        assert_eq!(update.kind, ChangeKind::Update);
        assert!(update.old_tuple.is_some());
        assert!(update.new_tuple.is_some());

        let mut delete = vec![b'D'];
        delete.extend_from_slice(&7_u32.to_be_bytes());
        delete.push(b'O');
        delete.extend_from_slice(&tuple);
        let PgOutputMessage::Change(delete) = PgOutputDecoder::decode(&delete).unwrap_or_else(|_| unreachable!()) else { unreachable!() };
    ...
    #[test]
    fn decodes_update_without_old_tuple_and_truncate() {
        let tuple = tuple_text(b"n");
        let mut update = vec![b'U'];
        update.extend_from_slice(&9_u32.to_be_bytes());
        update.push(b'N');
        update.extend_from_slice(&tuple);
        let PgOutputMessage::Change(change) = PgOutputDecoder::decode(&update).unwrap_or_else(|_| unreachable!()) else { unreachable!() };
        ...
        let PgOutputMessage::Change(change) = PgOutputDecoder::decode(&truncate).unwrap_or_else(|_| unreachable!()) else { unreachable!() };
        assert_eq!(change.kind, ChangeKind::Truncate);
        assert_eq!(change.relation_id, 9);
    }

    #[test]
    fn rejects_unknown_truncated_and_malformed_messages() {
        ...
        assert_eq!(PgOutputDecoder::decode(b"B"), Err(PgOutputError::UnexpectedEof));

        let mut bad_relation = vec![b'R'];
        bad_relation.extend_from_slice(&1_u32.to_be_bytes());
        bad_relation.extend_from_slice(b"p\0t\0");
        bad_relation.push(b'x');
        bad_relation.extend_from_slice(&0_u16.to_be_bytes());
        assert_eq!(PgOutputDecoder::decode(&bad_relation), Err(PgOutputError::InvalidReplicaIdentity(b'x')));

        let mut insert = vec![b'I'];
        insert.extend_from_slice(&1_u32.to_be_bytes());
        insert.push(b'K');
        assert_eq!(PgOutputDecoder::decode(&insert), Err(PgOutputError::InvalidTupleTag(b'K')));
    }

    #[test]
    fn rejects_tuple_bounds_and_trailing_bytes() {
        let mut too_many = vec![b'I'];
        too_many.extend_from_slice(&1_u32.to_be_bytes());
        too_many.push(b'N');
        too_many.extend_from_slice(&(1025_u16).to_be_bytes());
        assert_eq!(PgOutputDecoder::decode(&too_many), Err(PgOutputError::TooManyColumns(1025)));
        ...
        begin.push(0);
        assert_eq!(PgOutputDecoder::decode(&begin), Err(PgOutputError::TrailingBytes(1)));
    }
}
