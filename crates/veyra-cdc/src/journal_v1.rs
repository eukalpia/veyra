...
/// Detects duplicate WAL delivery and rejects conflicting content at the same commit LSN.
...
/// Append-only transaction journal. `append` returns only after data is synced to the filesystem.
pub struct Journal {
    path: PathBuf,
    file: File,
    guard: ReplayGuard,
...
impl Journal {
    ...
    pub fn replay(&mut self) -> Result<Vec<TransactionBatch>, JournalError> {
        ...
        scan(&mut reader, false).map(|(records, _)| records)
    }
...
fn scan(file: &mut File, allow_incomplete_tail: bool) -> Result<(Vec<TransactionBatch>, Option<u64>), JournalError> {
    ...
    loop {
        ...
        if remaining < header_len {
            return incomplete_tail(records, offset, allow_incomplete_tail);
        }
        let mut header = [0_u8; HEADER_LEN];
        ...
        let payload_len = read_u64_header(&header[8..16])?;
        let commit_lsn = read_u64_header(&header[16..24])?;
        let fingerprint = read_u64_header(&header[24..32])?;
...
fn read_u64_header(bytes: &[u8]) -> Result<u64, JournalError> {
    let array: [u8; 8] = bytes.try_into().map_err(|_| JournalError::CorruptHeader)?;
...
fn encode_batch(batch: &TransactionBatch) -> Result<Vec<u8>, JournalError> {
    ...
    let count_u32 = u32::try_from(count).map_err(|_| JournalError::TooManyChanges(count))?;
    let mut out = Vec::new();
...
fn decode_batch(bytes: &[u8]) -> Result<TransactionBatch, JournalError> {
    ...
    let count_raw = cursor.u32()?;
    let count = usize::try_from(count_raw).map_err(|_| JournalError::LengthOverflow)?;
    if count > MAX_CHANGES { return Err(JournalError::TooManyChanges(count)); }
...
struct LittleCursor<'a> { bytes: &'a [u8], offset: usize }
impl<'a> LittleCursor<'a> {
    ...
    fn optional_bytes(&mut self) -> Result<Option<Vec<u8>>, JournalError> {
        match self.u8()? {
            ...
            1 => {
                let raw = self.u32()?;
                let len = usize::try_from(raw).map_err(|_| JournalError::LengthOverflow)?;
...
/// Portable software CRC32C (Castagnoli). Runtime SIMD can replace this without format changes.
...
mod tests {
    ...
    #[test]
    fn journal_round_trips_reopens_and_duplicate_is_noop() -> Result<(), JournalError> {
    ...
    #[test]
    fn recovery_truncates_only_incomplete_final_record() -> Result<(), JournalError> {
    ...
    fn checksum_corruption_is_rejected() -> Result<(), Box<dyn std::error::Error>> {
        ...
        { let mut file = OpenOptions::new().read(true).write(true).open(&path)?; file.seek(SeekFrom::Start(len - 1))?; let mut byte = [0_u8; 1]; file.read_exact(&mut byte)?; file.seek(SeekFrom::Start(len - 1))?; file.write_all(&[byte[0] ^ 0xff])?; file.sync_data()?; }
        let error = Journal::open(&path).err().ok_or("expected checksum error")?;
