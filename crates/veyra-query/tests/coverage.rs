...
fn party(count: u32) -> BookingParty {
    ...
    for id in 1..=count {
        builder.add_traveler(Traveler::new(
            TravelerId::new(id), AgeEvidence::AgeAtCheckIn(30), false,
...
fn document(room_id: u32, capacity: u16, start: i32) -> RoomDocument {
    RoomDocument {
        ...
        occupancy_adjustment: OccupancyAdjustment {
            per_adult_per_night: money(0), per_child_per_night: money(0),
        },
        distance_meters: 100,
        quality_milli: 700,
...
fn engine(room: RoomDocument, start_day: u32) -> SearchEngine {
    let mut availability = AvailabilityIndex::new(start_day, 8, 1)
        .unwrap_or_else(|_| unreachable!());
...
fn query<'a>(party: &'a BookingParty, start: u32) -> StayQuery<'a> {
    StayQuery {
        destination_id: 7,
        check_in_day: start,
...
fn catalog_and_query_validation_fail_closed() {
    ...
    assert_eq!(SearchEngine::try_new(availability, vec![non_dense]), Err(QueryError::InvalidAdultAge(0)));

    let party = party(1);
    let engine = engine(document(0, 2, 10), 10);
    let mut request = query(&party, 10); request.limit = 0;
    assert_eq!(engine.search(&request), Err(QueryError::InvalidLimit(0)));
    let mut request = query(&party, 10); request.check_out_day = request.check_in_day;
    assert_eq!(engine.search(&request), Err(QueryError::InvalidStayRange));
    let mut request = query(&party, 10); request.budget = Some(MoneyMicros::signed(-1));
...
fn service_day_pricing_occupancy_and_ranking_errors_propagate() {
    ...
    let engine = engine(document(0, 4, i32::MAX), huge);
    assert_eq!(engine.search(&query(&party, huge)), Err(QueryError::ServiceDayOutOfRange));

    let occupancy_engine = engine(document(0, 1, 10), 10);
    let result = occupancy_engine.search(&query(&party, 10)).unwrap_or_else(|_| unreachable!());
    ...
    assert_eq!(
        engine(negative, 10).search(&query(&party, 10)),
        Err(QueryError::Pricing(PricingError::NegativeTotal))
    );

    let mut bad_rank = document(0, 4, 10); bad_rank.quality_milli = 1_001;
    assert_eq!(
        engine(bad_rank, 10).search(&query(&party, 10)),
...
fn all_query_error_display_variants_are_stable() {
    let errors = [
        ...
        QueryError::RoomDocumentCountMismatch { expected: 1, actual: 2 },
        QueryError::NonDenseRoomId { expected: 1, actual: 2 },
        QueryError::InvalidAdultAge(1), QueryError::MissingRoomProjection(1),
        QueryError::InvalidLimit(1), QueryError::InvalidStayRange,
        QueryError::ServiceDayOutOfRange, QueryError::NegativeBudget,
