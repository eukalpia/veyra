...
impl OccupancyContext {
    ...
    #[must_use]
    pub fn occupants(&self) -> &[Occupant] { &self.occupants }
...
pub enum Rule {
    ...
    AgeRangeCount { min_age: u16, max_age: u16, min_count: u16, max_count: u16 },
...
impl Rule {
    ...
    fn validate_inner(&self, nodes: &mut usize) -> Result<(), RuleError> {
        *nodes = nodes.checked_add(1).ok_or(RuleError::RuleTooComplex)?;
        if *nodes > MAX_RULE_NODES { return Err(RuleError::RuleTooComplex); }
        match self {
            Self::Capacity { min, max } if min > max => Err(RuleError::InvalidCountRange),
            Self::AgeRangeCount { min_age, max_age, min_count, max_count } if min_age > max_age || min_count > max_count => Err(RuleError::InvalidCountRange),
            Self::RequireAdult { adult_age, min_adults } if *adult_age == 0 || *min_adults == 0 => Err(RuleError::InvalidAdultRule),
            Self::RequireGuardianForMinors { minor_below_age } if *minor_below_age == 0 => Err(RuleError::InvalidGuardianRule),
            Self::And(children) | Self::Or(children) => {
                if children.is_empty() { return Err(RuleError::EmptyBooleanRule); }
                for child in children { child.validate_inner(nodes)?; }
                Ok(())
            }
    ...
    fn evaluate_validated(&self, context: &OccupancyContext) -> Result<bool, RuleError> {
        match self {
            Self::Capacity { min, max } => {
                let count = u16::try_from(context.occupants.len()).map_err(|_| RuleError::CountOverflow)?;
                Ok((*min..=*max).contains(&count))
            }
            Self::AgeRangeCount { min_age, max_age, min_count, max_count } => {
                let count = u16::try_from(context.count_age_range(*min_age, *max_age)).map_err(|_| RuleError::CountOverflow)?;
                Ok((*min_count..=*max_count).contains(&count))
            }
            Self::RequireAdult { adult_age, min_adults } => {
                let count = context.occupants.iter().filter(|occupant| occupant.age >= *adult_age).count();
                Ok(count >= usize::from(*min_adults))
            }
            Self::RequireGuardianForMinors { minor_below_age } => Ok(context.occupants.iter().all(|occupant| {
                occupant.age >= *minor_below_age || occupant.authorized_guardian_present
            })),
            Self::And(children) => {
                for child in children {
                    if !child.evaluate_validated(context)? { return Ok(false); }
                }
                Ok(true)
            }
            Self::Or(children) => {
                for child in children {
                    if child.evaluate_validated(context)? { return Ok(true); }
                }
                Ok(false)
            }
...
mod tests {
    ...
    fn occupant(id: u32, age: u16, guardian: bool) -> Occupant {
        Occupant { traveler_id: TravelerId::new(id), age, authorized_guardian_present: guardian }
    }
    ...
    fn hard_rules_evaluate_deterministically() {
        ...
        assert_eq!(Rule::Capacity { min: 1, max: 2 }.evaluate(&context), Ok(true));
        assert_eq!(Rule::RequireAdult { adult_age: 18, min_adults: 1 }.evaluate(&context), Ok(true));
        assert_eq!(Rule::AgeRangeCount { min_age: 0, max_age: 17, min_count: 1, max_count: 1 }.evaluate(&context), Ok(true));
        assert_eq!(Rule::RequireGuardianForMinors { minor_below_age: 18 }.evaluate(&context), Ok(true));
        assert_eq!(context.occupants().len(), 2);
    }

    #[test]
    fn guardian_rule_fails_closed_for_unsupervised_minor() {
        let context = OccupancyContext::new(vec![occupant(1, 12, false)]).unwrap_or_else(|_| unreachable!());
        assert_eq!(Rule::RequireGuardianForMinors { minor_below_age: 18 }.evaluate(&context), Ok(false));
    }

    #[test]
    fn boolean_ir_composes_without_dynamic_code() {
        let context = OccupancyContext::new(vec![occupant(1, 30, false)]).unwrap_or_else(|_| unreachable!());
    ...
    fn invalid_ir_is_rejected_before_execution() {
        assert_eq!(Rule::Capacity { min: 2, max: 1 }.validate(), Err(RuleError::InvalidCountRange));
        assert_eq!(Rule::AgeRangeCount { min_age: 10, max_age: 5, min_count: 0, max_count: 1 }.validate(), Err(RuleError::InvalidCountRange));
        assert_eq!(Rule::RequireAdult { adult_age: 0, min_adults: 1 }.validate(), Err(RuleError::InvalidAdultRule));
        assert_eq!(Rule::RequireGuardianForMinors { minor_below_age: 0 }.validate(), Err(RuleError::InvalidGuardianRule));
        assert_eq!(Rule::And(Vec::new()).validate(), Err(RuleError::EmptyBooleanRule));
        assert_eq!(Rule::Or(Vec::new()).validate(), Err(RuleError::EmptyBooleanRule));
    }

    #[test]
    fn occupancy_context_validates_identity_and_bounds() {
        assert_eq!(OccupancyContext::new(Vec::new()), Err(RuleError::InvalidOccupantCount(0)));
        assert_eq!(OccupancyContext::new(vec![occupant(1, 1, false), occupant(1, 2, false)]), Err(RuleError::DuplicateTraveler(TravelerId::new(1))));
        let context = OccupancyContext::new(vec![occupant(1, 5, false), occupant(2, 20, false), occupant(3, 70, false)]).unwrap_or_else(|_| unreachable!());
